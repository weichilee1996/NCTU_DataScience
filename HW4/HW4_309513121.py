...
def inference(opt):
    ...
    for j in range(0,100):
        Tensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
        z = Variable(Tensor(np.random.normal(0, 1, (opt.inference_num, opt.latent_dim))))
        gen_imgs = generator(z)
        for i in tqdm(range(0, opt.inference_num)):
            if j == 0:
                save_image(gen_imgs.data[i],'result/images_inference/%d.png'%i,normalize=True)
            elif j != 0:
                a = j*100 + i
                save_image(gen_imgs.data[i],'result/images_inference/%d.png'%a,normalize=True)
            #save_image(gen_imgs.data[i],"result/images_inference/%d.png" % i,normalize=True)
